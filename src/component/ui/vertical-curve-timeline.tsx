...
export const VerticalCurveTimeline = ({ milestones }: { milestones: TimelineMilestone[] }) => {
  ...
  const { scrollYProgress: desktopScrollProgress } = useScroll({
    target: containerRef,
    offset: ["start 50%", "end 50%"] // Changed from "start 85%", "end center" for faster progression
  });
  
  // Mobile timeline scroll tracking with adjusted offsets
  const { scrollYProgress: mobileScrollProgress } = useScroll({
    target: mobileContainerRef,
    offset: ["start 50%", "end 50%"] // Changed from "start 95%", "end 20%" for faster progression
  });
  ...
  return (
    <div className="relative">
      ...
      <div className="relative min-h-[1400px] hidden md:block" ref={containerRef}>
